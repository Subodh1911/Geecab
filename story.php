<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
<title>Indasto Industries HTML Template</title>
<!-- Stylesheets -->
<link href="css/bootstrap.css" rel="stylesheet">
<link href="css/style.css" rel="stylesheet">
<!-- Responsive File -->
<link href="css/responsive.css" rel="stylesheet">
<!-- Color File -->
<link href="css/color.css" rel="stylesheet">

<link rel="preconnect" href="https://fonts.gstatic.com">
<link href="https://fonts.googleapis.com/css2?family=Heebo:wght@400;500;600;700;800&family=Mukta:wght@500;600;700;800&display=swap" rel="stylesheet">

<link rel="shortcut icon" href="images/favicon.png" type="image/x-icon">
<link rel="icon" href="images/favicon.png" type="image/x-icon">

<!-- Responsive -->
<meta http-equiv="X-UA-Compatible" content="IE=edge">
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0">
<!--[if lt IE 9]><script src="https://cdnjs.cloudflare.com/ajax/libs/html5shiv/3.7.3/html5shiv.js"></script><![endif]-->
<!--[if lt IE 9]><script src="js/respond.js"></script><![endif]-->
</head>

<body>

<div class="page-wrapper">

    <!-- Main Header -->
    <header class="main-header header-style-one">

        <!-- Header Top -->
        <div class="header-top">
            <div class="auto-container">
                <div class="inner-container">
                    <div class="left-column">
                        <div class="location">
                            <form action="#" class="location-switcher">
                                <select class="selectpicker">
                                    <option value="1">Los Angeles</option>
                                    <option value="2">New York</option>
                                    <option value="3">Chicago</option>
                                    <option value="4">Boston</option>
                                </select>
                            </form>
                        </div>
                        <ul class="contact-info">
                            <li><a href="tel:+08963648018"><i class="flaticon-phone-call"></i>+089 636-48018</a></li>
                            <li><a href="mailto:info@indasto.com"><i class="flaticon-envelope"></i>info@indasto.com</a></li>
                            <li><i class="flaticon-location-pointer"></i> 191 Integer Rd, 08219</li>
                        </ul>              
                    </div>
                    <div class="right-column"> 
                        <ul class="social-links">
                            <li><a href="#"><i class="fab fa-facebook-f"></i></a></li>
                            <li><a href="#"><i class="fab fa-twitter"></i></a></li>
                            <li><a href="#"><i class="fab fa-google-plus-g"></i></a></li>
                            <li><a href="#"><i class="fab fa-youtube"></i></a></li>
                        </ul>
                        <div class="get-quote-btn"><a href="#">Get a Quote</a></div>
                    </div>
                </div>
            </div>
        </div>

        <!-- Header Upper -->
        <div class="header-upper">
            <div class="auto-container">
                <div class="inner-container">
                    <!--Logo-->
                    <div class="logo-box">
                        <div class="logo"><a href="index.php"><img src="images/logo.png" alt=""></a></div>
                    </div>
                    <div class="right-column">
                        <!--Nav Box-->
                        <div class="nav-outer">
                            <!--Mobile Navigation Toggler-->
                            <div class="mobile-nav-toggler"><img src="images/icons/icon-bar-3.png" alt=""></div>

                            <!-- Main Menu -->
                            <nav class="main-menu navbar-expand-md navbar-light">
                                <div class="collapse navbar-collapse show clearfix" id="navbarSupportedContent">
                                    <ul class="navigation">
                                        <li><a href="index.php">Home</a></li>
                                        <li class="dropdown"><a href="#">Pages</a>
                                            <ul>
                                                <li><a href="about.php">About Us</a></li>
                                                <li><a href="story.php">Our Story</a></li>
                                                <li><a href="team-3.php">Our Leaders</a></li>
                                                <li><a href="testimonial-2.php">Testimonials</a></li>
                                                <li><a href="faq.php">FAQ’s</a></li>
                                            </ul>
                                        </li>
                                        <li><a href="service.php">Products</a></li>
                                        <li><a href="blog.php">Blog</a></li>
                                        <li><a href="contact.php">Contact</a></li>
                                        <li><a href="gallery.php">Gallery</a></li>
                                        <li><a href="event.php">Event</a></li>
                                    </ul>
                                </div>
                            </nav>
                        </div>
                        <div class="navbar-right-info">
                            <div class="search-toggler"><i class="flaticon-search-1"></i></div>
                            <div class="side-menu-nav sidemenu-nav-toggler"><img src="images/icons/icon-bar-3.png" alt=""></div>
                        </div>
                    </div>                        
                </div>
            </div>
        </div>
        <!--End Header Upper-->

        <!-- Sticky Header  -->
        <div class="sticky-header">
            <div class="header-upper">
                <div class="auto-container">
                    <div class="inner-container">
                        <!--Logo-->
                        <div class="logo-box">
                            <div class="logo"><a href="index.php"><img src="images/logo.png" alt=""></a></div>
                        </div>
                        <div class="right-column">
                            <!--Nav Box-->
                            <div class="nav-outer">
                                <!--Mobile Navigation Toggler-->
                                <div class="mobile-nav-toggler"><img src="images/icons/icon-bar-3.png" alt=""></div>
    
                                <!-- Main Menu -->
                                <nav class="main-menu navbar-expand-md navbar-light">
                                </nav>
                            </div>
                            <div class="navbar-right-info">
                                <div class="search-toggler"><i class="flaticon-search-1"></i></div>
                                <div class="side-menu-nav sidemenu-nav-toggler"><img src="images/icons/icon-bar-3.png" alt=""></div>
                            </div>
                        </div>                        
                    </div>
                </div>
            </div>
        </div><!-- End Sticky Menu -->

        <!-- Mobile Menu  -->
        <div class="mobile-menu">
            <div class="menu-backdrop"></div>
            <div class="close-btn"><span class="icon flaticon-remove"></span></div>
            
            <nav class="menu-box">
                <div class="nav-logo"><a href="index.php"><img src="images/logo-2.png" alt="" title=""></a></div>
                <div class="menu-outer"><!--Here Menu Will Come Automatically Via Javascript / Same Menu as in Header--></div>
				<!--Social Links-->
				<div class="social-links">
					<ul class="clearfix">
						<li><a href="#"><span class="fab fa-twitter"></span></a></li>
						<li><a href="#"><span class="fab fa-facebook-square"></span></a></li>
						<li><a href="#"><span class="fab fa-pinterest-p"></span></a></li>
						<li><a href="#"><span class="fab fa-instagram"></span></a></li>
						<li><a href="#"><span class="fab fa-youtube"></span></a></li>
					</ul>
                </div>
            </nav>
        </div><!-- End Mobile Menu -->

        <div class="nav-overlay">
            <div class="cursor"></div>
            <div class="cursor-follower"></div>
        </div>
    </header>
    <!-- End Main Header -->

    <!-- Hidden Sidebar -->
    <section class="hidden-sidebar close-sidebar">
        <div class="wrapper-box">
            <div class="content-wrapper">
                <div class="hidden-sidebar-close"><span class="flaticon-close"></span></div>
                <div class="text-widget sidebar-widget">
                    <div class="logo"><a href="index.php"><img src="images/logo-2.png" alt=""></a></div>
                    <div class="text">Demoralized by the charms of pleasure of the moment so blinded by desire that they cannot foresees the pain.</div>
                </div>
                <div class="new-widget-two widget">
                    <h4 class="widget_title">News & Updates</h4>
                    <div class="content-widget">
                        <article class="post">
                            <figure class="post-thumb"><a href="blog-details.php"><img src="images/resource/image-85.jpg" alt=""></a></figure>
                            <div class="content">
                                <div class="post-info"><i class="far fa-clock"></i> Oct 15, 2020</div>
                                <h5><a href="blog-details.php">Gas shield solution developed for the aerospace</a></h5>
                            </div>
                        </article>
                        <article class="post">
                            <figure class="post-thumb"><a href="blog-details.php"><img src="images/resource/image-86.jpg" alt=""></a></figure>
                            <div class="content">
                                <div class="post-info"><i class="far fa-clock"></i> Sep 26, 2020</div>
                                <h5><a href="blog-details.php">Building back a sustainable  manufacturing sector</a></h5>
                            </div>
                        </article>
                    </div>
                </div>
                <div class="brochure-widget widget">
                    <h4 class="widget_title">Our Materials</h4>
                    <div class="widget-content">                                
                        <div class="single-brochure"><a href="#"><i class="flaticon-pdf-file-format-symbol"></i> Service Details.pdf</a></div>
                        <div class="single-brochure"><a href="#"><i class="flaticon-doc-file-format"></i>Company Reports.doc</a></div>
                    </div>
                </div>
                <div class="copyright-text">Copyright © 2021 <a href="#"> Indasto.</a> All Rights Reserved.</div>
            </div>
        </div>
    </section>
    
    <!--Search Popup-->
    <div id="search-popup" class="search-popup">
        <div class="close-search theme-btn"><span class="flaticon-remove"></span></div>
        <div class="popup-inner">
            <div class="overlay-layer"></div>
            <div class="search-form">
                <form method="post" action="index.php">
                    <div class="form-group">
                        <fieldset>
                            <input type="search" class="form-control" name="search-input" value="" placeholder="Search Here" required >
                            <input type="submit" value="Search Now!" class="theme-btn">
                        </fieldset>
                    </div>
                </form>
            </div>
        </div>
    </div>

    <!-- Page Title -->
    <section class="page-title" style="background-image: url(images/background/bg-26.jpg);">
        <div class="auto-container">
            <div class="content-box">
                <div class="content-wrapper">
                    <div class="title">
                        <h1>Our Story</h1>
                    </div>
                    <ul class="bread-crumb style-two">
                        <li><a href="index.php">Home </a></li>
                        <li><a href="#">Pages </a></li>
                        <li class="active">Our Story</li>
                    </ul>
                </div>
            </div>
        </div>
    </section>

    <!-- About section Five -->
    <section class="about-section-five">
        <div class="auto-container">
            <div class="row">
                <div class="col-lg-6 image-column">
                    <div class="image"><img src="images/resource/image-7.jpg" width="100%" alt=""></div>
                    <!-- <div class="title-box">
                        <div class="sec-title mb-0">
                            <div class="sub-title">About Company</div>
                            <h2>Best Industrial Solution <br> Provider <span></span></h2>
                        </div>
                    </div> -->
                </div>
                <div class="col-lg-6">
                    <!-- <div class="image"><img src="images/resource/image-88.jpg" alt=""></div> -->
                    <div class="text">
                        <p>It is a long established fact that a readers will be distracted by readable content of a page when looking at its layout. The point of using is that it has a more-or-less normal distribution.</p>
                        <p>It is a long established fact that a readers will be distracted by readable content of a page when looking at its layout.</p>
                    </div>
                    <div class="author-information">                        
                        <ul class="contact-info">
                            <li><img src="images/resource/author-11.jpg" alt=""></li>
                            <li><a href="tel:+08963648018"><i class="flaticon-phone-call"></i><span>+089 636-48018</span></a></li>
                            <li><a href="mailto:info@Indasto.com"><i class="flaticon-envelope"></i><span>info@Indasto.com</span></a></li>
                        </ul>
                        <div class="author-info">
                            <h4>Simon Charley</h4>
                            <div class="designation">Ceo & Founder</div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>

    <!-- Statement section -->
    <section class="statement-section">
        <div class="auto-container">
            <div class="row no-gutters">
                <div class="col-xl-4 statement-block">
                    <div class="inner-box" style="background-image: url(images/resource/image-107.jpg);">
                        <div class="icon-outer">
                            <div class="icon"><img src="images/icons/icon-37.png" alt=""></div>
                        </div>
                        <h4>Mission Statement</h4>
                        <div class="text">Explain too you how all this get mistaken idea denounce pleasures <br>
                            off give you completed well.
                        </div>
                        <div class="link"><a href="#">Read More <i class="flaticon-right-arrow-3"></i></a></div>
                    </div>
                </div>
                <div class="col-xl-4 statement-block">
                    <div class="inner-box" style="background-image: url(images/resource/image-108.jpg);">
                        <div class="icon-outer">
                            <div class="icon"><img src="images/icons/icon-38.png" alt=""></div>
                        </div>
                        <h4>Vission Statement</h4>
                        <div class="text">Denouncing pleasure and praising pain was born and I will give you a <br>
                            account of the systemof the great explorer. 
                        </div>
                        <div class="link"><a href="#">Read More <i class="flaticon-right-arrow-3"></i></a></div>
                    </div>
                </div>
                <div class="col-xl-4 statement-block">
                    <div class="inner-box" style="background-image: url(images/resource/image-109.jpg);">
                        <div class="icon-outer">
                            <div class="icon"><img src="images/icons/icon-39.png" alt=""></div>
                        </div>
                        <h4>Corporate Value</h4>
                        <div class="text">Our power of choice is untrammelled and when nothing prevents our <br>
                            being able to do what we like best.
                        </div>
                        <div class="link"><a href="#">Read More <i class="flaticon-right-arrow-3"></i></a></div>
                    </div>
                </div>
            </div>
        </div>
    </section>

    <!-- Our Facts section Three -->
    <section class="our-facts-section-three style-two" style="background-image: url(images/background/bg-28.jpg);">
        <div class="auto-container">
            <div class="row">
                <div class="col-lg-3 col-md-6 column facts-block">
                    <div class="inner wow fadeInLeft" data-wow-delay="0ms" data-wow-duration="1500ms">
                        <div class="content">                            
                            <div class="icon-two"><i class="flaticon-car"></i></div>
                            <div class="count-outer count-box">
                                <span class="count-text" data-speed="3000" data-stop="6.4">0</span><span>K</span>
                            </div>
                            <div class="text">Projects <br> Done Successfully</div>
                            <div class="icon"><i class="flaticon-car"></i></div>
                            <div class="link"><a href="#" class="link-btn">Our Projects <i class="flaticon-right-arrow-3"></i></a></div>
                        </div>
                    </div>
                </div>
                <div class="col-lg-3 col-md-6 column facts-block">
                    <div class="inner wow fadeInLeft" data-wow-delay="0ms" data-wow-duration="1500ms">
                        <div class="content">                            
                            <div class="icon-two"><i class="flaticon-factory-3"></i></div>
                            <div class="count-outer count-box">
                                <span>+</span><span class="count-text" data-speed="3000" data-stop="49">0</span>
                            </div>
                            <div class="text">Branches <br>  Running Worldwide</div>
                            <div class="icon"><i class="flaticon-factory-3"></i></div>
                            <div class="link"><a href="#" class="link-btn">Our Locations<i class="flaticon-right-arrow-3"></i></a></div>
                        </div>
                    </div>
                </div>
                <div class="col-lg-3 col-md-6 column facts-block">
                    <div class="inner wow fadeInLeft" data-wow-delay="0ms" data-wow-duration="1500ms">
                        <div class="content">                            
                            <div class="icon-two"><i class="flaticon-engineer-2"></i></div>
                            <div class="count-outer count-box">
                                <span class="count-text" data-speed="3000" data-stop="1.3">0</span><span>K</span>
                            </div>
                            <div class="text">Expert <br>
                                Team Members
                                </div>
                            <div class="icon"><i class="flaticon-engineer-2"></i></div>
                            <div class="link"><a href="#" class="link-btn">Meet Our Team <i class="flaticon-right-arrow-3"></i></a></div>
                        </div>
                    </div>
                </div>
                <div class="col-lg-3 col-md-6 column facts-block">
                    <div class="inner wow fadeInLeft" data-wow-delay="0ms" data-wow-duration="1500ms">
                        <div class="content">                            
                            <div class="icon-two"><i class="flaticon-users"></i></div>
                            <div class="count-outer count-box">
                                <span>+</span><span class="count-text" data-speed="3000" data-stop="14">0</span>
                            </div>
                            <div class="text">Happy & Satisfied
                                Customers
                            </div>
                            <div class="icon"><i class="flaticon-users"></i></div>
                            <div class="link"><a href="#" class="link-btn">Happy Customer <i class="flaticon-right-arrow-3"></i></a></div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>

    <!-- History section -->
    <section class="history-section">
        <div class="auto-container">
            <div class="sec-title text-center">
                <div class="sub-title">Our History</div>
                <h2>Reason for Choosing Us</h2>
                <div class="text">We have facility to produce advance work various industrial applications <br> based on specially developed technology.</div>
            </div>
            <div class="nav-tabs-wrapper">
                <div class="nav nav-tabs tab-btn-style-two">
                    <div class="theme_carousel owl-theme owl-carousel" data-options='{"loop": false, "margin": 0, "autoheight":true, "lazyload":true, "nav": true, "dots": true, "autoplay": true, "autoplayTimeout": 6000, "smartSpeed": 1000, "responsive":{ "0" :{ "items": "1" }, "600" :{ "items" : "2" }, "768" :{ "items" : "3" } , "992":{ "items" : "4" }, "1200":{ "items" : "4" }}}'>
                        <ul class="item">
                            <li><a class="active" data-toggle="tab" href="#tab-one">
                                <h4>2014</h4>
                            </a></li>
                        </ul>
                        <ul class="item">
                            <li><a data-toggle="tab" href="#tab-two">
                                <h4>2015</h4>
                            </a></li>
                        </ul>
                        <ul class="item">
                            <li><a data-toggle="tab" href="#tab-three">
                                <h4>2016</h4>
                            </a></li>
                        </ul>
                        <ul class="item">
                            <li><a class="active" data-toggle="tab" href="#tab-four">
                                <h4>2017</h4>
                            </a></li>
                        </ul>
                        <ul class="item">
                            <li><a data-toggle="tab" href="#tab-five">
                                <h4>2018</h4>
                            </a></li>
                        </ul>
                        <ul class="item">
                            <li><a data-toggle="tab" href="#tab-six">
                                <h4>2020</h4>
                            </a></li>
                        </ul>
                    </div>
                </div>
            </div>
            
            <div class="tab-content">
                <div class="tab-pane fadeInUp animated" id="tab-one" role="tabpanel" aria-labelledby="tab-one">
                    <div class="theme_carousel owl-theme owl-carousel" data-options='{"loop": false, "margin": 30, "autoheight":true, "lazyload":true, "nav": true, "dots": true, "autoplay": true, "autoplayTimeout": 6000, "smartSpeed": 1000, "responsive":{ "0" :{ "items": "1" }, "600" :{ "items" : "1" }, "768" :{ "items" : "1" } , "992":{ "items" : "1" }, "1200":{ "items" : "2" }}}'>
                        <div class="history-block">
                            <div class="row no-gutters">
                                <div class="col-md-4">
                                    <div class="image"><img src="images/resource/image-110.jpg" alt=""></div>
                                </div>
                                <div class="col-md-8">
                                    <div class="content">
                                        <h4>The Company was Founded <br> in Los Angeles</h4>
                                        <div class="text">Indignation & dislike men who are so beguiled demoralized by the charms of pleasure of the moment owing to the claims of duty or the obligations of business accepted.</div>
                                        <ul class="list">
                                            <li>Beguiled and demoralized by the charms</li>
                                            <li>Equal blame belongs to those who fail in their prevents</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="history-block">
                            <div class="row no-gutters">
                                <div class="col-md-4">
                                    <div class="image"><img src="images/resource/image-111.jpg" alt=""></div>
                                </div>
                                <div class="col-md-8">
                                    <div class="content">
                                        <h4>The Company was Founded <br> in Los Angeles</h4>
                                        <div class="text">Indignation & dislike men who are so beguiled demoralized by the charms of pleasure of the moment owing to the claims of duty or the obligations of business accepted.</div>
                                        <ul class="list">
                                            <li>Beguiled and demoralized by the charms</li>
                                            <li>Equal blame belongs to those who fail in their prevents</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="history-block">
                            <div class="row no-gutters">
                                <div class="col-md-4">
                                    <div class="image"><img src="images/resource/image-110.jpg" alt=""></div>
                                </div>
                                <div class="col-md-8">
                                    <div class="content">
                                        <h4>The Company was Founded <br> in Los Angeles</h4>
                                        <div class="text">Indignation & dislike men who are so beguiled demoralized by the charms of pleasure of the moment owing to the claims of duty or the obligations of business accepted.</div>
                                        <ul class="list">
                                            <li>Beguiled and demoralized by the charms</li>
                                            <li>Equal blame belongs to those who fail in their prevents</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="history-block">
                            <div class="row no-gutters">
                                <div class="col-md-4">
                                    <div class="image"><img src="images/resource/image-111.jpg" alt=""></div>
                                </div>
                                <div class="col-md-8">
                                    <div class="content">
                                        <h4>The Company was Founded <br> in Los Angeles</h4>
                                        <div class="text">Indignation & dislike men who are so beguiled demoralized by the charms of pleasure of the moment owing to the claims of duty or the obligations of business accepted.</div>
                                        <ul class="list">
                                            <li>Beguiled and demoralized by the charms</li>
                                            <li>Equal blame belongs to those who fail in their prevents</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>                    
                </div>
                <div class="tab-pane fadeInUp animated active" id="tab-two" role="tabpanel" aria-labelledby="tab-two">
                    <div class="theme_carousel owl-theme owl-carousel" data-options='{"loop": false, "margin": 30, "autoheight":true, "lazyload":true, "nav": true, "dots": true, "autoplay": true, "autoplayTimeout": 6000, "smartSpeed": 1000, "responsive":{ "0" :{ "items": "1" }, "600" :{ "items" : "1" }, "768" :{ "items" : "1" } , "992":{ "items" : "1" }, "1200":{ "items" : "2" }}}'>
                        <div class="history-block">
                            <div class="row no-gutters">
                                <div class="col-md-4">
                                    <div class="image"><img src="images/resource/image-110.jpg" alt=""></div>
                                </div>
                                <div class="col-md-8">
                                    <div class="content">
                                        <h4>The Company was Founded <br> in Los Angeles</h4>
                                        <div class="text">Indignation & dislike men who are so beguiled demoralized by the charms of pleasure of the moment owing to the claims of duty or the obligations of business accepted.</div>
                                        <ul class="list">
                                            <li>Beguiled and demoralized by the charms</li>
                                            <li>Equal blame belongs to those who fail in their prevents</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="history-block">
                            <div class="row no-gutters">
                                <div class="col-md-4">
                                    <div class="image"><img src="images/resource/image-111.jpg" alt=""></div>
                                </div>
                                <div class="col-md-8">
                                    <div class="content">
                                        <h4>The Company was Founded <br> in Los Angeles</h4>
                                        <div class="text">Indignation & dislike men who are so beguiled demoralized by the charms of pleasure of the moment owing to the claims of duty or the obligations of business accepted.</div>
                                        <ul class="list">
                                            <li>Beguiled and demoralized by the charms</li>
                                            <li>Equal blame belongs to those who fail in their prevents</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="history-block">
                            <div class="row no-gutters">
                                <div class="col-md-4">
                                    <div class="image"><img src="images/resource/image-110.jpg" alt=""></div>
                                </div>
                                <div class="col-md-8">
                                    <div class="content">
                                        <h4>The Company was Founded <br> in Los Angeles</h4>
                                        <div class="text">Indignation & dislike men who are so beguiled demoralized by the charms of pleasure of the moment owing to the claims of duty or the obligations of business accepted.</div>
                                        <ul class="list">
                                            <li>Beguiled and demoralized by the charms</li>
                                            <li>Equal blame belongs to those who fail in their prevents</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="history-block">
                            <div class="row no-gutters">
                                <div class="col-md-4">
                                    <div class="image"><img src="images/resource/image-111.jpg" alt=""></div>
                                </div>
                                <div class="col-md-8">
                                    <div class="content">
                                        <h4>The Company was Founded <br> in Los Angeles</h4>
                                        <div class="text">Indignation & dislike men who are so beguiled demoralized by the charms of pleasure of the moment owing to the claims of duty or the obligations of business accepted.</div>
                                        <ul class="list">
                                            <li>Beguiled and demoralized by the charms</li>
                                            <li>Equal blame belongs to those who fail in their prevents</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="tab-pane fadeInUp animated" id="tab-three" role="tabpanel" aria-labelledby="tab-three">
                    <div class="theme_carousel owl-theme owl-carousel" data-options='{"loop": false, "margin": 30, "autoheight":true, "lazyload":true, "nav": true, "dots": true, "autoplay": true, "autoplayTimeout": 6000, "smartSpeed": 1000, "responsive":{ "0" :{ "items": "1" }, "600" :{ "items" : "1" }, "768" :{ "items" : "1" } , "992":{ "items" : "1" }, "1200":{ "items" : "2" }}}'>
                        <div class="history-block">
                            <div class="row no-gutters">
                                <div class="col-md-4">
                                    <div class="image"><img src="images/resource/image-110.jpg" alt=""></div>
                                </div>
                                <div class="col-md-8">
                                    <div class="content">
                                        <h4>The Company was Founded <br> in Los Angeles</h4>
                                        <div class="text">Indignation & dislike men who are so beguiled demoralized by the charms of pleasure of the moment owing to the claims of duty or the obligations of business accepted.</div>
                                        <ul class="list">
                                            <li>Beguiled and demoralized by the charms</li>
                                            <li>Equal blame belongs to those who fail in their prevents</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="history-block">
                            <div class="row no-gutters">
                                <div class="col-md-4">
                                    <div class="image"><img src="images/resource/image-111.jpg" alt=""></div>
                                </div>
                                <div class="col-md-8">
                                    <div class="content">
                                        <h4>The Company was Founded <br> in Los Angeles</h4>
                                        <div class="text">Indignation & dislike men who are so beguiled demoralized by the charms of pleasure of the moment owing to the claims of duty or the obligations of business accepted.</div>
                                        <ul class="list">
                                            <li>Beguiled and demoralized by the charms</li>
                                            <li>Equal blame belongs to those who fail in their prevents</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="history-block">
                            <div class="row no-gutters">
                                <div class="col-md-4">
                                    <div class="image"><img src="images/resource/image-110.jpg" alt=""></div>
                                </div>
                                <div class="col-md-8">
                                    <div class="content">
                                        <h4>The Company was Founded <br> in Los Angeles</h4>
                                        <div class="text">Indignation & dislike men who are so beguiled demoralized by the charms of pleasure of the moment owing to the claims of duty or the obligations of business accepted.</div>
                                        <ul class="list">
                                            <li>Beguiled and demoralized by the charms</li>
                                            <li>Equal blame belongs to those who fail in their prevents</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="history-block">
                            <div class="row no-gutters">
                                <div class="col-md-4">
                                    <div class="image"><img src="images/resource/image-111.jpg" alt=""></div>
                                </div>
                                <div class="col-md-8">
                                    <div class="content">
                                        <h4>The Company was Founded <br> in Los Angeles</h4>
                                        <div class="text">Indignation & dislike men who are so beguiled demoralized by the charms of pleasure of the moment owing to the claims of duty or the obligations of business accepted.</div>
                                        <ul class="list">
                                            <li>Beguiled and demoralized by the charms</li>
                                            <li>Equal blame belongs to those who fail in their prevents</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="tab-pane fadeInUp animated" id="tab-four" role="tabpanel" aria-labelledby="tab-four">
                    <div class="theme_carousel owl-theme owl-carousel" data-options='{"loop": false, "margin": 30, "autoheight":true, "lazyload":true, "nav": true, "dots": true, "autoplay": true, "autoplayTimeout": 6000, "smartSpeed": 1000, "responsive":{ "0" :{ "items": "1" }, "600" :{ "items" : "1" }, "768" :{ "items" : "1" } , "992":{ "items" : "1" }, "1200":{ "items" : "2" }}}'>
                        <div class="history-block">
                            <div class="row no-gutters">
                                <div class="col-md-4">
                                    <div class="image"><img src="images/resource/image-110.jpg" alt=""></div>
                                </div>
                                <div class="col-md-8">
                                    <div class="content">
                                        <h4>The Company was Founded <br> in Los Angeles</h4>
                                        <div class="text">Indignation & dislike men who are so beguiled demoralized by the charms of pleasure of the moment owing to the claims of duty or the obligations of business accepted.</div>
                                        <ul class="list">
                                            <li>Beguiled and demoralized by the charms</li>
                                            <li>Equal blame belongs to those who fail in their prevents</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="history-block">
                            <div class="row no-gutters">
                                <div class="col-md-4">
                                    <div class="image"><img src="images/resource/image-111.jpg" alt=""></div>
                                </div>
                                <div class="col-md-8">
                                    <div class="content">
                                        <h4>The Company was Founded <br> in Los Angeles</h4>
                                        <div class="text">Indignation & dislike men who are so beguiled demoralized by the charms of pleasure of the moment owing to the claims of duty or the obligations of business accepted.</div>
                                        <ul class="list">
                                            <li>Beguiled and demoralized by the charms</li>
                                            <li>Equal blame belongs to those who fail in their prevents</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="history-block">
                            <div class="row no-gutters">
                                <div class="col-md-4">
                                    <div class="image"><img src="images/resource/image-110.jpg" alt=""></div>
                                </div>
                                <div class="col-md-8">
                                    <div class="content">
                                        <h4>The Company was Founded <br> in Los Angeles</h4>
                                        <div class="text">Indignation & dislike men who are so beguiled demoralized by the charms of pleasure of the moment owing to the claims of duty or the obligations of business accepted.</div>
                                        <ul class="list">
                                            <li>Beguiled and demoralized by the charms</li>
                                            <li>Equal blame belongs to those who fail in their prevents</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="history-block">
                            <div class="row no-gutters">
                                <div class="col-md-4">
                                    <div class="image"><img src="images/resource/image-111.jpg" alt=""></div>
                                </div>
                                <div class="col-md-8">
                                    <div class="content">
                                        <h4>The Company was Founded <br> in Los Angeles</h4>
                                        <div class="text">Indignation & dislike men who are so beguiled demoralized by the charms of pleasure of the moment owing to the claims of duty or the obligations of business accepted.</div>
                                        <ul class="list">
                                            <li>Beguiled and demoralized by the charms</li>
                                            <li>Equal blame belongs to those who fail in their prevents</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="tab-pane fadeInUp animated" id="tab-five" role="tabpanel" aria-labelledby="tab-one">
                    <div class="theme_carousel owl-theme owl-carousel" data-options='{"loop": false, "margin": 30, "autoheight":true, "lazyload":true, "nav": true, "dots": true, "autoplay": true, "autoplayTimeout": 6000, "smartSpeed": 1000, "responsive":{ "0" :{ "items": "1" }, "600" :{ "items" : "1" }, "768" :{ "items" : "1" } , "992":{ "items" : "1" }, "1200":{ "items" : "2" }}}'>
                        <div class="history-block">
                            <div class="row no-gutters">
                                <div class="col-md-4">
                                    <div class="image"><img src="images/resource/image-110.jpg" alt=""></div>
                                </div>
                                <div class="col-md-8">
                                    <div class="content">
                                        <h4>The Company was Founded <br> in Los Angeles</h4>
                                        <div class="text">Indignation & dislike men who are so beguiled demoralized by the charms of pleasure of the moment owing to the claims of duty or the obligations of business accepted.</div>
                                        <ul class="list">
                                            <li>Beguiled and demoralized by the charms</li>
                                            <li>Equal blame belongs to those who fail in their prevents</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="history-block">
                            <div class="row no-gutters">
                                <div class="col-md-4">
                                    <div class="image"><img src="images/resource/image-111.jpg" alt=""></div>
                                </div>
                                <div class="col-md-8">
                                    <div class="content">
                                        <h4>The Company was Founded <br> in Los Angeles</h4>
                                        <div class="text">Indignation & dislike men who are so beguiled demoralized by the charms of pleasure of the moment owing to the claims of duty or the obligations of business accepted.</div>
                                        <ul class="list">
                                            <li>Beguiled and demoralized by the charms</li>
                                            <li>Equal blame belongs to those who fail in their prevents</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="history-block">
                            <div class="row no-gutters">
                                <div class="col-md-4">
                                    <div class="image"><img src="images/resource/image-110.jpg" alt=""></div>
                                </div>
                                <div class="col-md-8">
                                    <div class="content">
                                        <h4>The Company was Founded <br> in Los Angeles</h4>
                                        <div class="text">Indignation & dislike men who are so beguiled demoralized by the charms of pleasure of the moment owing to the claims of duty or the obligations of business accepted.</div>
                                        <ul class="list">
                                            <li>Beguiled and demoralized by the charms</li>
                                            <li>Equal blame belongs to those who fail in their prevents</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="history-block">
                            <div class="row no-gutters">
                                <div class="col-md-4">
                                    <div class="image"><img src="images/resource/image-111.jpg" alt=""></div>
                                </div>
                                <div class="col-md-8">
                                    <div class="content">
                                        <h4>The Company was Founded <br> in Los Angeles</h4>
                                        <div class="text">Indignation & dislike men who are so beguiled demoralized by the charms of pleasure of the moment owing to the claims of duty or the obligations of business accepted.</div>
                                        <ul class="list">
                                            <li>Beguiled and demoralized by the charms</li>
                                            <li>Equal blame belongs to those who fail in their prevents</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="tab-pane fadeInUp animated" id="tab-six" role="tabpanel" aria-labelledby="tab-two">
                    <div class="theme_carousel owl-theme owl-carousel" data-options='{"loop": false, "margin": 30, "autoheight":true, "lazyload":true, "nav": true, "dots": true, "autoplay": true, "autoplayTimeout": 6000, "smartSpeed": 1000, "responsive":{ "0" :{ "items": "1" }, "600" :{ "items" : "1" }, "768" :{ "items" : "1" } , "992":{ "items" : "1" }, "1200":{ "items" : "2" }}}'>
                        <div class="history-block">
                            <div class="row no-gutters">
                                <div class="col-md-4">
                                    <div class="image"><img src="images/resource/image-110.jpg" alt=""></div>
                                </div>
                                <div class="col-md-8">
                                    <div class="content">
                                        <h4>The Company was Founded <br> in Los Angeles</h4>
                                        <div class="text">Indignation & dislike men who are so beguiled demoralized by the charms of pleasure of the moment owing to the claims of duty or the obligations of business accepted.</div>
                                        <ul class="list">
                                            <li>Beguiled and demoralized by the charms</li>
                                            <li>Equal blame belongs to those who fail in their prevents</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="history-block">
                            <div class="row no-gutters">
                                <div class="col-md-4">
                                    <div class="image"><img src="images/resource/image-111.jpg" alt=""></div>
                                </div>
                                <div class="col-md-8">
                                    <div class="content">
                                        <h4>The Company was Founded <br> in Los Angeles</h4>
                                        <div class="text">Indignation & dislike men who are so beguiled demoralized by the charms of pleasure of the moment owing to the claims of duty or the obligations of business accepted.</div>
                                        <ul class="list">
                                            <li>Beguiled and demoralized by the charms</li>
                                            <li>Equal blame belongs to those who fail in their prevents</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="history-block">
                            <div class="row no-gutters">
                                <div class="col-md-4">
                                    <div class="image"><img src="images/resource/image-110.jpg" alt=""></div>
                                </div>
                                <div class="col-md-8">
                                    <div class="content">
                                        <h4>The Company was Founded <br> in Los Angeles</h4>
                                        <div class="text">Indignation & dislike men who are so beguiled demoralized by the charms of pleasure of the moment owing to the claims of duty or the obligations of business accepted.</div>
                                        <ul class="list">
                                            <li>Beguiled and demoralized by the charms</li>
                                            <li>Equal blame belongs to those who fail in their prevents</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="history-block">
                            <div class="row no-gutters">
                                <div class="col-md-4">
                                    <div class="image"><img src="images/resource/image-111.jpg" alt=""></div>
                                </div>
                                <div class="col-md-8">
                                    <div class="content">
                                        <h4>The Company was Founded <br> in Los Angeles</h4>
                                        <div class="text">Indignation & dislike men who are so beguiled demoralized by the charms of pleasure of the moment owing to the claims of duty or the obligations of business accepted.</div>
                                        <ul class="list">
                                            <li>Beguiled and demoralized by the charms</li>
                                            <li>Equal blame belongs to those who fail in their prevents</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>

    

    <!-- About section two -->
    <section class="about-section-two style-two">
        <div class="sec-bg" style="background-image: url(images/background/bg-29.jpg);"></div>
        <div class="auto-container">
            <div class="wrapper-box">
                <div class="row">
                    <div class="col-lg-6">
                        <div class="sec-title mb-30">
                            <div class="sub-title">About Company</div>
                            <h2>Leading Industrial Solution <br> Provider <span>Since 1988</span></h2>
                            <div class="text">It is a long established fact that a readers will be distracted by readable  content tof a page when looking at its layout. The point of using is that it has a more-or-less normal distribution.</div>
                        </div>
                        <div class="link-btn">
                            <a href="#" class="theme-btn style-two"><span>Read More</span></a>
                            <a href="#" class="theme-btn style-seven"><i class="flaticon-download-1"></i><span> Our Report</span></a>
                        </div>
                    </div>
                    <div class="col-lg-6">   
                        <div class="content-right">
                            <div class="text mb-30">
                                On the other hand, we denounce with righteous indignation and dislike men who are so beguiled.
                            </div>
                            <div class="icon-box">
                                <h4>Reduce Your Cost</h4>
                                <div class="text">Find fault with a man who chooses to enjoy. </div>
                                <div class="skills">
                                    <!--Skill Item-->
                                    <div class="skill-item">
                                        <div class="skill-bar">
                                            <div class="bar-inner"><div class="bar progress-line" data-width="84">
                                                <div class="skill-percentage"><div class="count-box"><span class="count-text" data-speed="2000" data-stop="84">0</span>%</div></div>
                                            </div></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="icon-box">
                                <h4>Shorten Delivery Time</h4>
                                <div class="text">Undertakes laborious physical exercise except to obtain. </div>
                                <div class="skills">
                                    <!--Skill Item-->
                                    <div class="skill-item">
                                        <div class="skill-bar">
                                            <div class="bar-inner"><div class="bar progress-line" data-width="65">
                                                <div class="skill-percentage"><div class="count-box"><span class="count-text" data-speed="2000" data-stop="65">0</span>%</div></div>
                                            </div></div>
                                        </div>
                                    </div>
                                </div>
                            </div> 
                        </div>                
                    </div>
                </div>
            </div>
        </div>
    </section>

    <!-- Whychoose us section -->
    <section class="whychoose-us-section-four">
        <div class="auto-container">
            <div class="sec-title text-center">
                <div class="sub-title">Highlights</div>
                <h2>Reason for Choosing Us</h2>
                <div class="text">We have facility to produce advance work various industrial applications <br> based on specially developed technology.</div>
            </div>
            <div class="wrapper-box">
                <div class="row">
                    <div class="col-lg-4 col-md-6 why-choose-us-block-four">
                        <div class="inner-box">
                            <div class="icon"><img src="images/icons/icon-13.png" alt=""></div>
                            <h4>On Time Delivery</h4>
                            <div class="text">Indignation dislike beguiled demoralized by the charms of pleasure blinded.</div>
                        </div>
                    </div>
                    <div class="col-lg-4 col-md-6 why-choose-us-block-four">
                        <div class="inner-box">
                            <div class="icon"><img src="images/icons/icon-14.png" alt=""></div>
                            <h4>Smart Technology</h4>
                            <div class="text">
                                Foresee the pain & trouble
                                that are bound ensue equal
                                blame their duty.
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-4 col-md-6 why-choose-us-block-four">
                        <div class="inner-box">
                            <div class="icon"><img src="images/icons/icon-15.png" alt=""></div>
                            <h4>Easy & Affordable</h4>
                            <div class="text">These cases are perfectly simple easy too distinguish  free hour prevents.</div>
                        </div>
                    </div>
                    <div class="col-lg-4 col-md-6 why-choose-us-block-four">
                        <div class="inner-box">
                            <div class="icon"><img src="images/icons/icon-40.png" alt=""></div>
                            <h4>Qualified Specialists</h4>
                            <div class="text">Foresee the pain & trouble
                                that are bound ensue equal
                                blame their duty.</div>
                        </div>
                    </div>
                    <div class="col-lg-4 col-md-6 why-choose-us-block-four">
                        <div class="inner-box">
                            <div class="icon"><img src="images/icons/icon-41.png" alt=""></div>
                            <h4>Clean & Unpolluted</h4>
                            <div class="text">These cases are perfectly simple easy too distinguish  free hour prevents.</div>
                        </div>
                    </div>
                    <div class="col-lg-4 col-md-6 why-choose-us-block-four">
                        <div class="inner-box">
                            <div class="icon"><img src="images/icons/icon-42.png" alt=""></div>
                            <h4>24/7  Support</h4>
                            <div class="text">Indignation dislike beguiled demoralized by the charms of pleasure blinded.</div>
                        </div>
                    </div>
                </div>
            </div>
            
        </div>
    </section>

    <!-- Team Section two style three -->
    <section class="team-section-two style-three">
        <div class="auto-container">
            <div class="sec-title">
                <div class="sub-title">Our Leaders</div>
                <h2>Team Behind Our Success </h2>
            </div>
            <div class="row">
                <div class="theme_carousel owl-theme owl-carousel" data-options='{"loop": true, "center": false, "margin": 0, "autoheight":true, "lazyload":true, "nav": true, "dots": true, "autoplay": true, "autoplayTimeout": 6000, "smartSpeed": 1000, "responsive":{ "0" :{ "items": "1" }, "480" :{ "items" : "1" }, "768" :{ "items" : "2" } , "992":{ "items" : "3" }, "1200":{ "items" : "3" }}}'>
                    <div class="col-lg-12 team-block-two">
                        <div class="inner-box">
                            <div class="image"><img src="images/resource/image-7.jpg" alt=""></div>
                            <div class="designation">Founder & CEO</div>
                            <div class="overlay">
                                <div>
                                    <h4>Boris Evan</h4>
                                    <div class="text">Equaly blame belongs to <br> those who fail their duty through <br> weak shrinking.</div>
                                    <ul class="social-icon">
                                        <li><a href="#"><i class="fab fa-facebook-f"></i></a></li>
                                        <li><a href="#"><i class="fab fa-twitter"></i></a></li>
                                        <li><a href="#"><i class="fab fa-linkedin-in"></i></a></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-12 team-block-two">
                        <div class="inner-box">
                            <div class="image"><img src="images/resource/image-8.jpg" alt=""></div>
                            <div class="designation">Founder & CEO</div>
                            <div class="overlay">
                                <div>
                                    <h4>Boris Evan</h4>
                                    <div class="text">Equaly blame belongs to <br> those who fail their duty through <br> weak shrinking.</div>
                                    <ul class="social-icon">
                                        <li><a href="#"><i class="fab fa-facebook-f"></i></a></li>
                                        <li><a href="#"><i class="fab fa-twitter"></i></a></li>
                                        <li><a href="#"><i class="fab fa-linkedin-in"></i></a></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-12 team-block-two">
                        <div class="inner-box">
                            <div class="image"><img src="images/resource/image-9.jpg" alt=""></div>
                            <div class="designation">Founder & CEO</div>
                            <div class="overlay">
                                <div>
                                    <h4>Boris Evan</h4>
                                    <div class="text">Equaly blame belongs to <br> those who fail their duty through <br> weak shrinking.</div>
                                    <ul class="social-icon">
                                        <li><a href="#"><i class="fab fa-facebook-f"></i></a></li>
                                        <li><a href="#"><i class="fab fa-twitter"></i></a></li>
                                        <li><a href="#"><i class="fab fa-linkedin-in"></i></a></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-12 team-block-two">
                        <div class="inner-box">
                            <div class="image"><img src="images/resource/image-7.jpg" alt=""></div>
                            <div class="designation">Founder & CEO</div>
                            <div class="overlay">
                                <div>
                                    <h4>Boris Evan</h4>
                                    <div class="text">Equaly blame belongs to <br> those who fail their duty through <br> weak shrinking.</div>
                                    <ul class="social-icon">
                                        <li><a href="#"><i class="fab fa-facebook-f"></i></a></li>
                                        <li><a href="#"><i class="fab fa-twitter"></i></a></li>
                                        <li><a href="#"><i class="fab fa-linkedin-in"></i></a></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-12 team-block-two">
                        <div class="inner-box">
                            <div class="image"><img src="images/resource/image-8.jpg" alt=""></div>
                            <div class="designation">Founder & CEO</div>
                            <div class="overlay">
                                <div>
                                    <h4>Boris Evan</h4>
                                    <div class="text">Equaly blame belongs to <br> those who fail their duty through <br> weak shrinking.</div>
                                    <ul class="social-icon">
                                        <li><a href="#"><i class="fab fa-facebook-f"></i></a></li>
                                        <li><a href="#"><i class="fab fa-twitter"></i></a></li>
                                        <li><a href="#"><i class="fab fa-linkedin-in"></i></a></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-12 team-block-two">
                        <div class="inner-box">
                            <div class="image"><img src="images/resource/image-9.jpg" alt=""></div>
                            <div class="designation">Founder & CEO</div>
                            <div class="overlay">
                                <div>
                                    <h4>Boris Evan</h4>
                                    <div class="text">Equaly blame belongs to <br> those who fail their duty through <br> weak shrinking.</div>
                                    <ul class="social-icon">
                                        <li><a href="#"><i class="fab fa-facebook-f"></i></a></li>
                                        <li><a href="#"><i class="fab fa-twitter"></i></a></li>
                                        <li><a href="#"><i class="fab fa-linkedin-in"></i></a></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-12 team-block-two">
                        <div class="inner-box">
                            <div class="image"><img src="images/resource/image-7.jpg" alt=""></div>
                            <div class="designation">Founder & CEO</div>
                            <div class="overlay">
                                <div>
                                    <h4>Boris Evan</h4>
                                    <div class="text">Equaly blame belongs to <br> those who fail their duty through <br> weak shrinking.</div>
                                    <ul class="social-icon">
                                        <li><a href="#"><i class="fab fa-facebook-f"></i></a></li>
                                        <li><a href="#"><i class="fab fa-twitter"></i></a></li>
                                        <li><a href="#"><i class="fab fa-linkedin-in"></i></a></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-12 team-block-two">
                        <div class="inner-box">
                            <div class="image"><img src="images/resource/image-8.jpg" alt=""></div>
                            <div class="designation">Founder & CEO</div>
                            <div class="overlay">
                                <div>
                                    <h4>Boris Evan</h4>
                                    <div class="text">Equaly blame belongs to <br> those who fail their duty through <br> weak shrinking.</div>
                                    <ul class="social-icon">
                                        <li><a href="#"><i class="fab fa-facebook-f"></i></a></li>
                                        <li><a href="#"><i class="fab fa-twitter"></i></a></li>
                                        <li><a href="#"><i class="fab fa-linkedin-in"></i></a></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-12 team-block-two">
                        <div class="inner-box">
                            <div class="image"><img src="images/resource/image-9.jpg" alt=""></div>
                            <div class="designation">Founder & CEO</div>
                            <div class="overlay">
                                <div>
                                    <h4>Boris Evan</h4>
                                    <div class="text">Equaly blame belongs to <br> those who fail their duty through <br> weak shrinking.</div>
                                    <ul class="social-icon">
                                        <li><a href="#"><i class="fab fa-facebook-f"></i></a></li>
                                        <li><a href="#"><i class="fab fa-twitter"></i></a></li>
                                        <li><a href="#"><i class="fab fa-linkedin-in"></i></a></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>

    <!-- CTA section -->
    <section class="cta-section">
        <div class="auto-container">
            <div class="row align-items-center">
                <div class="col-lg-7">
                    <h2>Delivering Results with Solid Dependability</h2>
                </div>
                <div class="col-lg-5">
                    <div class="link-btn">
                        <a href="#" class="theme-btn"><span>Get a Quote</span></a>
                        <a href="#" class="theme-btn style-three"><span><i class="flaticon-phone-call"></i> +089 636 - 48018</span></a>
                    </div>
                </div>
            </div>
        </div>
    </section>

    <!--Main Footer-->
    <footer class="main-footer">
        <div class="upper-box">
            <div class="auto-container">
                <div class="row">
                    <div class="col-lg-3 col-md-6">
                        <div class="widget about-widget">
                            <div class="logo"><a href="index.php"><img src="images/logo-2.png" alt=""></a></div>
                            <div class="text">Indasto is your one-call solution <br>  for a wide range of manufactory <br> and industry solutions.</div>
                            <ul class="social-links">
                                <li><a href="#"><i class="fab fa-facebook-f"></i></a></li>
                                <li><a href="#"><i class="fab fa-twitter"></i></a></li>
                                <li><a href="#"><i class="fab fa-google-plus-g"></i></a></li>
                                <li><a href="#"><i class="fab fa-youtube"></i></a></li>
                            </ul>
                        </div>
                    </div>
                    <div class="col-lg-3 col-md-6">
                        <div class="widget links-widget">
                            <h4 class="widget_title">Useful Links</h4>
                            <div class="widget-content">
                                <div class="row">
                                    <div class="col-md-6">
                                        <ul class="list">
                                            <li><a href="about.php">About Us</a></li>
                                            <li><a href="#">Services</a></li>
                                            <li><a href="#">Industries</a></li>
                                            <li><a href="blog.php">News</a></li>
                                            <li><a href="#">Projects</a></li>
                                        </ul>
                                    </div>
                                    <div class="col-md-6">
                                        <ul class="list">
                                            <li><a href="about.php">Industries</a></li>
                                            <li><a href="#">Features</a></li>
                                            <li><a href="#">Process</a></li>
                                            <li><a href="blog.php">Feedback</a></li>
                                            <li><a href="#">Contact</a></li>
                                        </ul>
                                    </div>
                                </div>                                
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-3 col-md-6">
                        <div class="contact-widget widget">
                            <h4 class="widget_title">Get In Touch</h4>
                            <ul>
                                <li><i class="flaticon-location-pointer"></i> 191 Integer Rd, 2nd Str <br>  LA 08219 USA.</li>
                                <li><i class="flaticon-clock-3"></i> 09 am to 05.30 pm <br> Mon - Sat [ Sun - Closed ] </li>
                                <li><i class="flaticon-envelope"></i> <a href="mailto:info@indasto.com">info@indasto.com</a> <br>  Online support & Quote  </li>
                            </ul>
                        </div>
                    </div>
                    <div class="col-lg-3 col-md-6">
                        <div class="instagram-widget widget">
                            <h4 class="widget_title">Our Gallery</h4>
                            <div class="wrapper-box">
                                <div class="image">
                                    <img src="images/resource/image-16.jpg" alt="">
                                    <div class="overlay-link"><a href="images/resource/image-16.jpg" class="lightbox-image" data-fancybox="gallery"><span class="flaticon-zoom"></span></a></div>
                                </div>
                                <div class="image">
                                    <img src="images/resource/image-17.jpg" alt="">
                                    <div class="overlay-link"><a href="images/resource/image-17.jpg" class="lightbox-image" data-fancybox="gallery"><span class="flaticon-zoom"></span></a></div>
                                </div>
                                <div class="image">
                                    <img src="images/resource/image-18.jpg" alt="">
                                    <div class="overlay-link"><a href="images/resource/image-18.jpg" class="lightbox-image" data-fancybox="gallery"><span class="flaticon-zoom"></span></a></div>
                                </div>
                                <div class="image">
                                    <img src="images/resource/image-19.jpg" alt="">
                                    <div class="overlay-link"><a href="images/resource/image-19.jpg" class="lightbox-image" data-fancybox="gallery"><span class="flaticon-zoom"></span></a></div>
                                </div>
                                <div class="image">
                                    <img src="images/resource/image-20.jpg" alt="">
                                    <div class="overlay-link"><a href="images/resource/image-20.jpg" class="lightbox-image" data-fancybox="gallery"><span class="flaticon-zoom"></span></a></div>
                                </div>
                                <div class="image">
                                    <img src="images/resource/image-21.jpg" alt="">
                                    <div class="overlay-link"><a href="images/resource/image-21.jpg" class="lightbox-image" data-fancybox="gallery"><span class="flaticon-zoom"></span></a></div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div> 
        </div>               
    </footer>
    <!--End Main Footer-->

    <div class="footer-bottom">
        <div class="auto-container">
            <div class="wrapper-box">
                <div class="row m-0 align-items-center justify-content-between">
                    <div class="copyright-text">Copyright © 2021 <a href="#"> Indasto.</a> All Rights Reserved.</div>
                    <ul class="menu">
                        <li><a href="#">Terms & Conditions</a></li>
                        <li><a href="#">Privacy Policy </a></li>
                        <li><a href="#">Sitemap</a></li>
                    </ul>
                </div>
            </div>
        </div>
    </div>
	
</div>
<!--End pagewrapper-->

<!--Scroll to top-->
<div class="scroll-to-top scroll-to-target" data-target="html"><span class="flaticon-arrow-1"></span></div>

<script src="js/jquery.js"></script>
<script src="js/popper.min.js"></script>
<script src="js/bootstrap.min.js"></script>
<script src="js/bootstrap-select.min.js"></script>
<script src="js/jquery.fancybox.js"></script>
<script src="js/isotope.js"></script>
<script src="js/owl.js"></script>
<script src="js/appear.js"></script>
<script src="js/wow.js"></script>
<script src="js/scrollbar.js"></script>
<script src="js/TweenMax.min.js"></script>
<script src="js/swiper.min.js"></script>
<script src="js/jquery.polyglot.language.switcher.js"></script>
<script src="js/jquery.ajaxchimp.min.js"></script>
<script src="js/parallax-scroll.js"></script>
<script src="js/knob.js"></script>

<script src="js/script.js"></script>


</body>
</html>